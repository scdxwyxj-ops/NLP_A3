...
DEFAULT_CE_POOL = Path(
    ...
    "dev_full_dev_full_dev_top64_strict_top64_candidates.json"
)
DEFAULT_DENSE_POOL = Path(
    "round18/outputs/o_dense/o_d1_light_biencoder/"
    "dev_full_dev_o_d1_light_biencoder_fallback_tfidf_svd_top500_candidates.json"
)
DEFAULT_OUTPUT_DIR = Path("round18/outputs/o_aggregate/o_a2_calibrated_rank")
DEFAULT_MANIFEST = Path("round18/outputs/o_aggregate/o_a2_calibrated_rank/run_manifest.json")
DEFAULT_RECORD_PATH = Path("round18/outputs/o_aggregate/o_a2_calibrated_rank/run_record.json")
DEFAULT_RUN_ID = "o_a2_calibrated_rank"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Round18 O-A2 calibrated leaf-flat ranking over sparse and CE candidate leaves "
    ...
    parser.add_argument(
        "--dense-pool",
        type=Path,
        default=DEFAULT_DENSE_POOL,
        help=(
            "Dense fallback pool from the same contract. Not used in fixed variants, "
            "but included for contract-level traceability."
        ),
    )
    parser.add_argument(
        "--output-dir",
...
def _enforce_strict_inputs(claims: Path, evidence: Path, sparse_pool: Path, ce_pool: Path, dense_pool: Path) -> None:
    if claims != DEFAULT_CLAIMS:
        raise SystemExit("O-A2 is defined for dev-only: claims must be data/dev-claims.json.")
    if evidence != DEFAULT_EVIDENCE:
        raise SystemExit("O-A2 requires data/evidence.json as evidence input.")
    if sparse_pool != DEFAULT_SPARSE_POOL:
        raise SystemExit(
            "O-A2 requires current-run strict sparse pool at: "
            f"{DEFAULT_SPARSE_POOL}"
        )
    if ce_pool != DEFAULT_CE_POOL:
        raise SystemExit(
            "O-A2 requires current-run cross-encoder top64 pool at: "
            f"{DEFAULT_CE_POOL}"
        )
    if dense_pool != DEFAULT_DENSE_POOL:
        raise SystemExit(
            "O-A2 requires current-run dense fallback pool at: "
            f"{DEFAULT_DENSE_POOL}"
        )
...
def build_input_records(args: argparse.Namespace, claims_path: Path, evidence_path: Path) -> list[dict[str, Any]]:
    ...
    return [
        ...
        {
            "path": str(DEFAULT_DENSE_POOL),
            "sha256": sha256_file(DEFAULT_DENSE_POOL),
            "split": "current_run_artifact",
            "labels_used": False,
        },
    ]
...
def main() -> None:
    ...
    base_command = "python round18/experiments/o_aggregate/o_a2_calibrated_rank/run_o_a2_calibrated_rank.py"

    _enforce_strict_inputs(args.claims, args.evidence, args.sparse_pool, args.ce_pool, args.dense_pool)
    ...
    for path in (args.claims, args.evidence, args.sparse_pool, args.ce_pool, args.dense_pool):
        if not path.exists():
    ...
    dense_pool, dense_stats = _parse_pool(args.dense_pool, claims, args.candidate_k)

    # Precompute candidate text tokens for the candidate universe used for MMR.
    tokenized_evidence: dict[str, tuple[str, ...]] = {}
    candidate_evidence_ids: set[str] = set()
    for pool in (sparse_pool, ce_pool, dense_pool):
    ...
    data_flow = {
        ...
        "inputs": {
            ...
            "dense_pool": str(args.dense_pool),
        },
        "input_file_hashes": {
    ...
    command = (
        ...
        f"--sparse-pool {args.sparse_pool} --ce-pool {args.ce_pool} --dense-pool {args.dense_pool} "

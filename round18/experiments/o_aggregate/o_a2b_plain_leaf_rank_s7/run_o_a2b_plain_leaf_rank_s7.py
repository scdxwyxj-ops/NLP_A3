...
DEFAULT_OUTPUT_DIR = Path("round18/outputs/o_aggregate/o_a2b_plain_leaf_rank_s7")
DEFAULT_MANIFEST = Path("round18/outputs/o_aggregate/o_a2b_plain_leaf_rank_s7/run_manifest.json")
DEFAULT_RECORD_PATH = Path("round18/outputs/o_aggregate/o_a2b_plain_leaf_rank_s7/run_record.json")
DEFAULT_RUN_ID = "o_a2b_plain_leaf_rank_s7"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Round18 O-A2b plain leaf-flat ranking over O-S7 sparse and O-D3b CE candidate leaves "
    ...
    parser.add_argument(
        "--stage",
        default="o_a2b_plain_leaf_rank_s7",
...
def _enforce_strict_inputs(claims: Path, evidence: Path, sparse_pool: Path, ce_pool: Path) -> None:
    if claims != DEFAULT_CLAIMS:
        raise SystemExit("O-A2b is defined for dev-only: claims must be data/dev-claims.json.")
    if evidence != DEFAULT_EVIDENCE:
        raise SystemExit("O-A2b requires data/evidence.json as evidence input.")
    if sparse_pool != DEFAULT_SPARSE_POOL:
        raise SystemExit(
            "O-A2b requires current-run strict O-S7 sparse pool at: "
            f"{DEFAULT_SPARSE_POOL}"
        )
    if ce_pool != DEFAULT_CE_POOL:
        raise SystemExit(
            "O-A2b requires current-run O-D3b cross-encoder top64 pool at: "
            f"{DEFAULT_CE_POOL}"
        )
...
def main() -> None:
    ...
    base_command = "python round18/experiments/o_aggregate/o_a2b_plain_leaf_rank_s7/run_o_a2b_plain_leaf_rank_s7.py"

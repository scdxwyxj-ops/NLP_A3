...
DEFAULT_OUTPUT_DIR = Path("round18/reports/ce_shallow_feature_kfold")
DEFAULT_RUN_ID = "ce_shallow_feature_kfold"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train-only k-fold test for whether shallow hand features supplement "
            "CE/MiniLM scores in top64 context selection."
        )
    )
    ...
    parser.add_argument("--random-seed", type=int, default=1819)
...
def parse_ce_pool(path: Path, claims: dict[str, Any], candidate_k: int) -> dict[str, list[dict[str, Any]]]:
    if not path.exists():
            ...
            selected.append(
                {
                    ...
                    "ce_rank": coerce_int(row.get("rank"), position),
                    "ce_score": coerce_float(row.get("reranker_score", row.get("score", 0.0))),
...
def build_examples(
    ...
    for claim_id, claim in claims.items():
        ...
        for item in ce_pool.get(claim_id, []):
            ...
            features.update(
                {
                    ...
                    "ce_rank": float(item["ce_rank"]),
                    "ce_rr": 1.0 / (60.0 + float(item["ce_rank"])),
                }
            )
            rows.append(
                Example(
...
def feature_names_for_variant(variant: str) -> list[str]:
    ...
    if variant == "ce_plus_shallow":
        return ["ce_score", "ce_rank", "ce_rr"] + list(SHALLOW_FEATURES)
    if variant == "ce_plus_factual":
        factual = [
            name
        ...
        return ["ce_score", "ce_rank", "ce_rr"] + factual
...
def rank_examples(
    ...
    for claim_id, examples in examples_by_claim.items():
        if not examples:
            ...
            continue
        if variant == "ce_only":
            scores = np.asarray([example.features["ce_score"] for example in examples], dtype=np.float32)
        else:
            if ranker is None:
                raise ValueError("Non-CE variant requires a trained ranker.")
            scores = ranker.predict(matrix_for_examples(examples, feature_names))
        scored = sorted(
            zip(examples, scores),
            key=lambda item: (-float(item[1]), item[0].features.get("ce_rank", 10**9), item[0].evidence_id),
        )
            ...
            out.append(
                {
                    ...
                    "rank": rank,
                    "score": float(score),
                    "ce_score": float(example.features.get("ce_score", 0.0)),
                    "ce_rank": int(example.features.get("ce_rank", 0.0)),
...
def evaluate_recall_at_k(
    ...
    for k in ks:
        ...
        metrics[f"micro_recall@{k}"] = (
            totals[k]["tp"] / totals[k]["gold"] if totals[k]["gold"] else 0.0
        )
...
def make_report(
    ...
    best: dict[str, Any],
    ce_dev: dict[str, float],
    forbidden_hits: dict[str, list[str]],
    elapsed: float,
) -> None:
    delta = best.get("dev_macro_recall@64", 0.0) - ce_dev.get("macro_recall@64", 0.0)
    lines = [
        ...
        f"- CE-only dev macro@64: `{ce_dev.get('macro_recall@64', 0.0):.6f}`",
        f"- Dev delta vs CE-only: `{delta:+.6f}`",
        "",
        "## Leakage Scan",
...
def main() -> None:
    ...
    for required in (args.train_claims, args.dev_claims, args.evidence, args.train_ce_pool, args.dev_ce_pool):
    ...
    train_ce = parse_ce_pool(args.train_ce_pool, train_claims, args.candidate_k)
    dev_ce = parse_ce_pool(args.dev_ce_pool, dev_claims, args.candidate_k)
    train_examples = build_examples(train_claims, evidence, train_ce)
    dev_examples = build_examples(dev_claims, evidence, dev_ce)
    folds = split_folds(list(train_examples), args.k_folds, args.fold_seed)

    variants = ["ce_only", "shallow_only", "ce_plus_factual", "ce_plus_shallow"]
    rows: list[dict[str, Any]] = []
    train_ranked_by_variant: dict[str, dict[str, list[dict[str, Any]]]] = {}
    ...
    for variant in variants:
        ...
        feature_names = feature_names_for_variant(variant)
        ranker = None if variant == "ce_only" else train_ranker(train_examples, feature_names, args)
        train_ranked_by_variant[variant] = rank_examples(train_examples, variant, ranker, feature_names, args.candidate_k)
    ...
    best = rows[0]
    ce_dev_metrics = next(row for row in rows if row["variant"] == "ce_only")
    ...
    write_json(best_train_candidates_path, train_ranked_by_variant[best["variant"]])
    write_json(best_dev_candidates_path, dev_ranked_by_variant[best["variant"]])
    payload = {
        "run_id": args.run_id,
        "status": "strict-candidate",
        "selection_basis": "train_kfold",
    ...
    make_report(
        ...
        rows=rows,
        best=best,
        ce_dev={key.replace("dev_", ""): val for key, val in ce_dev_metrics.items() if key.startswith("dev_")},
    ...
    print(f"best_variant={best['variant']} cv@64={best.get('cv_macro_recall@64'):.6f} dev@64={best.get('dev_macro_recall@64'):.6f}")

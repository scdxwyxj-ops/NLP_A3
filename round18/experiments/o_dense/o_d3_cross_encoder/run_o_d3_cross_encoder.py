...
DEFAULT_OUTPUT_DIR = Path("round18/outputs/o_dense/o_d3_cross_encoder")
DEFAULT_RUN_ID = "o_d3_cross_encoder"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Round18 O-D3 strict bounded cross-encoder reranker over sparse candidates."
    )
    parser.add_argument(
        "--claims",
        type=Path,
...
def _enforce_contract(claims: Path, evidence: Path, sparse_pool: Path) -> None:
    if claims != DEFAULT_SPLIT_CLAIMS:
        raise SystemExit(
            "O-D3 in this scope is defined for dev claims only: claims must be data/dev-claims.json."
        )
    if evidence != DEFAULT_EVIDENCE:
        raise SystemExit("O-D3 in this scope requires data/evidence.json as evidence input.")
    if sparse_pool != DEFAULT_SPARSE_POOL:
        raise SystemExit(
            "O-D3 must use the current-run O-S4 sparse top-500 pool: "
...
def main() -> None:
    ...
    command = (
        f"python round18/experiments/o_dense/o_d3_cross_encoder/run_o_d3_cross_encoder.py "

...
vocabulary $V_g$.  Let $q_g(c)$ and $x_g(d)$ be the resulting count vectors for
claim $c$ and evidence document $d$.  For each gate, the notebook builds a
claim matrix $Q_g$ and an evidence matrix $X_g$, reweights the evidence matrix
with BM25, and computes batched sparse products $Q_gX_g^\top$ before retaining
the top-ranked evidence rows per claim.
...
for the sparse/embedding-prefiltered pool.  Candidate evidence text is truncated
before tokenization, then claim--evidence pairs are scored in PyTorch batches.
...
The sparse retrieval experiments showed that combining multiple sparse retrieval methods improved evidence recall compared to using only a single retrieval source. Figure~\ref{fig:sparse} shows that the fusion retriever achieved the highest macro recall of 67.5% at top-500. Character-based retrieval also performed strongly by itself with 66.5% recall, which suggests that climate claims benefited from partial word matching and scientific terminology matching.  Structured retrieval alone achieved lower recall, but it still slightly improved the overall fusion performance when combined with the other sparse retrieval methods. 

The staged retrieval pipeline also helped reduce computational cost while still preserving useful evidence candidates. The sparse top-500 stage achieved 67.5% macro recall, while the reranked top-64 stage retained 57.8% macro recall. The final submitted top 3 evidence achieved an F-score of 0.2021. Figure~\ref{fig:staging} shows the cost-benefit view of gradually reducing the candidate pool, and Figure~\ref{fig:recall-vs-k} shows the corresponding recall-vs-K behavior across retrieval stages. 

The reranking experiments showed that combining semantic embeddings with shallow factual cues improved evidence selection quality. Figure~\ref{fig:top64-selector} shows that the top-64 selector achieved the highest macro recall of 57.8%, outperforming both sparse cutoff retrieval and cross encoder plus cues. Figure~\ref{fig:top64-score-distribution} further shows that the fused top-64 score has a higher central value and lower dispersion than the individual embedding or shallow-feature scores, while Figure~\ref{fig:shallow-complement} shows where factual cues complement semantic scores. This suggests that semantic similarity alone was not enough for strong evidence ranking and that shallow factual features helped improve retrieval consistency. 

The classifier diagnostics in Figure~\ref{fig:final-classifier} show that the model performed best on the SUPPORTS class, while most classification errors occurred between SUPPORTS, REFUTES and NOT-ENOUGH-INFO. The DISPUTED class was the hardest to classify due to the smaller number of development examples and more ambiguous evidence relationships. 

The classifier ablation experiments in Table~\ref{tab:classifier-ablation} also presented that structured evidence formatting improved classification performance compared to a plain evidence sequence. Plain context achieved 0.4351% accuracy and 0.3762 macro F1, while the enhanced structured context improved performance to 0.4935 accuracy and 0.4544 macro F1. The best diagnostic classifier result was achieved with C = 0.125, reaching 0.5195 accuracy and 0.4841 macro F1. 
